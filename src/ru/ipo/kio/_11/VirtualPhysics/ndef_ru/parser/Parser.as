	...
	public class Parser {
		...
		public function parse(expr:String):Array{
			...
			while(pos<expression.length){
				if(isOperator()){
					if(isSign() && expected[6]==1){
						...
						if(expected[1]==0){
							error_parsing(pos, "непредвиденный оператор");
							return [];
						}
						noperators+=2;
						addfunc(tokenstack, operstack, TFUNC2);
					...
					if(expected[0]==0){
						error_parsing(pos, "непредвиденное число");
						return [];
					}
					...
					if(expected[0]==0){
						error_parsing(pos, "непредвиденная константа");
						return [];
					}
					...
					if(expected[3]==0){
						error_parsing(pos, "неизвестная функция");
						return [];
					}
					...
					if(expected[0]==0){
						error_parsing(pos, "неизвестная переменная");
						return [];
					}
					...
					if(errormsg==""){
						error_parsing(pos, "неизвестный символ");
					}
					else{
			...
			if(tmpprio<0 || tmpprio>=10){
				error_parsing(pos, "незакрытые скобки");
				return [];
			}
